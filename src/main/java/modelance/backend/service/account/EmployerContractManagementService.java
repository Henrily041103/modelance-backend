...
import modelance.backend.firebasedto.work.ContractStatusDTO;
...
public class EmployerContractManagementService {
    ...
    public String addContract(ContractModel contractModel) throws InterruptedException, ExecutionException {
        ...
        // Get Model reference
        DocumentReference modelRef = firestore.collection("Model").document(contractModel.getModel().getId());
        contractDTO.setModel(modelRef);

        // Get Job reference
        contractDTO.setJob(jobRef);

        // Get Contract status reference
        DocumentReference statRef = firestore.collection("ContractStatus").document(contractModel.getJob().getId());
        contractDTO.setStatus(statRef);
